...
def main():
    ...
    val_loader = DataLoader(val, batch_size=1, num_workers=16, pin_memory=False, shuffle=False)
    model = UNet3D_Lightning.load_from_checkpoint("/cluster/51/emre/project/name=0_epoch=11_val_avg_overall_dice=0.55.ckpt", 
    ...
    model.to(device)
    test(model, val_loader, experiment_name="double_unet", device=device)
